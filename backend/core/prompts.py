...
1. **summary 필드 (필수):**
   ...
   **반드시 다음 4개 섹션을 이모지 + 굵은 제목 형식으로 순서대로 포함해야 합니다:**
...
3. **scripts 필드 (필수):**
   ...
   - **to_company.body**: **⚠️ 필수 작성** - 현재 상황 요약, 문제가 되는 계약/조항 내용, criteria에서 참고한 표준계약서·법령과의 차이, 회사에 바라는 조치를 정중히 요청하는 메일 본문 (200자 이내, 빈 문자열 금지)
   - **to_advisor.subject**: 노무사·청년노동센터·고용노동부 등에 보내는 상담 요청 메일 제목 (50자 이내)
   - **to_advisor.body**: **⚠️ 필수 작성** - 사실관계(근로형태, 근무시간, 임금 지급 방식), 문제로 의심되는 조항, criteria에서 참고한 표준계약서/법령, 상담 받고 싶은 쟁점을 구조화해서 적어둔 메일 본문 (200자 이내, 빈 문자열 금지)
...
- **⚠️ 매우 중요: scripts 필드의 body는 반드시 작성해야 합니다. 빈 문자열("")로 두지 마세요.**
  - `to_company.body`: 현재 상황 요약, 문제가 되는 계약/조항 내용, criteria에서 참고한 표준계약서·법령과의 차이, 회사에 바라는 조치를 정중히 요청하는 내용을 포함하여 200자 이내로 작성하세요.
  - `to_advisor.body`: 사실관계(근로형태, 근무시간, 임금 지급 방식), 문제로 의심되는 조항, criteria에서 참고한 표준계약서/법령, 상담 받고 싶은 쟁점을 구조화해서 적어둔 내용을 200자 이내로 작성하세요.
"""
    return prompt
