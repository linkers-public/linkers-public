...
{{
    "summary": "마크다운 형식 리포트 (아래 4개 섹션을 정확한 형식으로 필수 포함)",
    "criteria": [
        {{
            "name": "판단 기준명",
...
1. **summary 필드 (필수):**
   **⚠️ 매우 중요: summary 필드는 반드시 아래 형식을 그대로 복사하여 사용해야 합니다. 섹션 제목(헤더) 문자열은 정확히 위와 똑같이 써야 합니다.**
   
   **반드시 다음 4개 섹션을 정확한 형식으로 순서대로 포함해야 합니다:**
   
   ```markdown
   ## 상황 분석의 결과
   
   [여기에 제공된 상황을 바탕으로 핵심 문제점과 위험도를 요약하여 2-3문장으로 설명]
   ...
   - [즉시 실행 가능한 구체적인 행동 방안 1]
   - [즉시 실행 가능한 구체적인 행동 방안 2]
   - [즉시 실행 가능한 구체적인 행동 방안 3]
   - [즉시 실행 가능한 구체적인 행동 방안 4]
   ...
   **⚠️ 매우 중요 - 섹션 헤더 정확성:**
   - 첫 번째 섹션: 반드시 `## 상황 분석의 결과` (정확히 이 문자열)
   - 두 번째 섹션: 반드시 `## 법적 관점에서 본 현재상황` (정확히 이 문자열)
   - 세 번째 섹션: 반드시 `## 지금 당장 할 수 있는 행동` (정확히 이 문자열)
   - 네 번째 섹션: 반드시 `## 이렇게 말해보세요` (정확히 이 문자열)
   - 마크다운 헤더(`##`)를 반드시 포함하세요.
...
3. **scripts 필드 (필수):**
   ...
   - **반드시 위의 "법적 근거 (criteria 생성용)" 섹션에서 제공된 criteria를 기반으로** 작성하세요.
   - 각 템플릿은 **200자 이내**로 간결하게 작성하세요. 너무 길게 작성하지 마세요.
   - JSON 형식:
     ```json
     "scripts": {{
       "to_company": {{
     ...
     ```
   - **to_company.subject**: 회사 인사/노무/팀장 등에게 보내는 공식·정중한 문의 메일 제목
   - **to_company.body**: 현재 상황 요약, 문제가 되는 계약/조항 내용, criteria에서 참고한 표준계약서·법령과의 차이, 회사에 바라는 조치를 정중히 요청하는 메일 본문 (200자 이내)
   - **to_advisor.subject**: 노무사·청년노동센터·고용노동부 등에 보내는 상담 요청 메일 제목
   - **to_advisor.body**: 사실관계(근로형태, 근무시간, 임금 지급 방식), 문제로 의심되는 조항, criteria에서 참고한 표준계약서/법령, 상담 받고 싶은 쟁점을 구조화해서 적어둔 메일 본문 (200자 이내)
   - **중요**: 위의 "법적 근거 (criteria 및 scripts 작성용)" 섹션에서 제공된 문서 제목(title), snippet, source_type을 활용하여 구체적인 법적 근거를 포함하세요.
   - **중요**: 본문(body)은 200자 이내로 간결하게 작성하세요. 너무 길게 작성하지 마세요.
   - **중요**: 제목(subject)은 50자 이내로 간결하게 작성하세요.
...
**[정답 예시] - 반드시 이 형식을 정확히 따라야 합니다:**

다음은 올바른 응답 형식의 예시입니다. **이 형식을 그대로 복사하여 사용하세요:**

```json
{{
  "summary": "## 상황 분석의 결과\\n\\n제공된 상황을 분석한 결과, 직장 내 괴롭힘 상황으로 판단됩니다. 상사가 부당한 지시와 모욕적 언행을 반복적으로 행한 것은 근로자에게 신체적·정신적 고통을 주는 행위에 해당할 가능성이 높습니다. 위험도는 75점으로 평가되며, 즉시 대응이 필요합니다.\\n\\n## 법적 관점에서 본 현재상황\\n\\n「직장 내 괴롭힘 판단 및 예방 대응 매뉴얼」에 따르면, 직장 내 괴롭힘은 업무상 지위나 관계를 이용하여 피해 근로자에게 신체적·정신적 고통을 주는 행위로 정의됩니다. 제공된 상황에서 상사가 부당한 지시와 모욕적 언행을 반복적으로 행한 것은 직장 내 괴롭힘에 해당할 가능성이 높습니다. 특히 매뉴얼에서 명시한 '반복적이고 지속적인 행위' 요건을 충족하는 것으로 보입니다. 직장 내 괴롭힘 방지 및 근로자 보호 등에 관한 법률에 따라 이러한 행위는 금지되어 있으며, 법적 처벌 대상이 될 수 있습니다.\\n\\n## 지금 당장 할 수 있는 행동\\n\\n- 괴롭힘 관련 증거 자료를 수집하세요 (대화 내용, 문자 메시지, 이메일 등)\\n- 대화 내용과 일시를 상세히 기록하세요\\n- 상황을 객관적으로 정리하여 문서화하세요\\n- 고용노동부 고객상담센터(1350)에 상담을 요청하세요\\n\\n## 이렇게 말해보세요\\n\\n회사에 보낼 문구: '안녕하세요. 최근 상사로부터 부당한 지시와 모욕적 언행을 반복적으로 받고 있어 직장 내 괴롭힘에 해당할 수 있다고 판단됩니다. 직장 내 괴롭힘 방지 및 근로자 보호 등에 관한 법률에 따라 이러한 행위는 금지되어 있습니다. 이에 대한 적절한 조치를 요청드립니다.'\\n\\n상담 기관에 보낼 문구: '안녕하세요. 직장 내 괴롭힘으로 인한 상담을 요청드립니다. 상사로부터 부당한 지시와 모욕적 언행을 반복적으로 받고 있으며, 이는 직장 내 괴롭힘 방지 및 근로자 보호 등에 관한 법률에 위반될 수 있습니다. 관련 증거 자료를 수집하여 보관 중이며, 법적 조치를 고려하고 있습니다.'",
  "criteria": [
    {{
      "name": "직장 내 괴롭힘 요건 충족 여부",
      "status": "likely",
      "reason": "「직장 내 괴롭힘 판단 및 예방 대응 매뉴얼」에 따르면, 직장 내 괴롭힘은 업무상 지위나 관계를 이용하여 피해 근로자에게 신체적·정신적 고통을 주는 행위로 정의됩니다. 제공된 상황에서 상사가 부당한 지시와 모욕적 언행을 반복적으로 행한 것은 직장 내 괴롭힘에 해당할 가능성이 높습니다. 특히 매뉴얼에서 명시한 '반복적이고 지속적인 행위' 요건을 충족하는 것으로 보입니다. 따라서 이 기준은 likely로 판단됩니다."
    }},
    {{
      "name": "법적 보호 대상 여부",
      "status": "likely",
      "reason": "직장 내 괴롭힘 방지 및 근로자 보호 등에 관한 법률에 따르면, 업무상 지위나 관계를 이용하여 근로자에게 신체적·정신적 고통을 주는 행위는 금지되어 있습니다. 제공된 상황에서 상사가 업무상 지위를 이용하여 부당한 지시와 모욕적 언행을 행한 것은 법적 보호 대상에 해당합니다. 따라서 이 기준은 likely로 판단됩니다."
    }}
  ],
  "action_plan": {{
    "steps": [
      {{
        "title": "증거 수집",
        "items": ["대화 내용 기록", "문자 메시지 및 이메일 보관", "증인 확보"]
      }},
      {{
        "title": "1차 대응",
        "items": ["회사에 상황 알림", "상담 기관 상담 요청"]
      }},
      {{
        "title": "상담/신고 루트",
        "items": ["고용노동부 고객상담센터(1350)", "국가인권위원회(1331)", "노무사 상담"]
      }}
    ]
  }},
  "scripts": {{
    "to_company": "안녕하세요. 최근 상사로부터 부당한 지시와 모욕적 언행을 반복적으로 받고 있어 직장 내 괴롭힘에 해당할 수 있다고 판단됩니다. 직장 내 괴롭힘 방지 및 근로자 보호 등에 관한 법률에 따라 이러한 행위는 금지되어 있습니다. 이에 대한 적절한 조치를 요청드립니다.",
    "to_advisor": "안녕하세요. 직장 내 괴롭힘으로 인한 상담을 요청드립니다. 상사로부터 부당한 지시와 모욕적 언행을 반복적으로 받고 있으며, 이는 직장 내 괴롭힘 방지 및 근로자 보호 등에 관한 법률에 위반될 수 있습니다. 관련 증거 자료를 수집하여 보관 중이며, 법적 조치를 고려하고 있습니다."
  }},
  "organizations": [
    {{
      "id": "moel_complaint",
      "name": "고용노동부 고객상담센터",
      "description": "직장 내 괴롭힘에 대한 신고와 상담을 제공하는 기관입니다.",
      "capabilities": ["신고", "상담"],
      "requiredDocs": ["증거자료", "근로계약서"],
      "legalBasis": "직장 내 괴롭힘 방지 및 근로자 보호 등에 관한 법률",
    ...
    {{
      "id": "human_rights",
      "name": "국가인권위원회",
      "description": "인권 보호와 상담을 제공하는 기관입니다.",
      "capabilities": ["상담"],
      "requiredDocs": [],
      "legalBasis": "국가인권위원회법",
    ...
    {{
      "id": "labor_attorney",
      "name": "노무사",
      "description": "직장 내 괴롭힘, 임금체불 등의 문제에 대한 상담을 제공하는 법인입니다.",
      "capabilities": ["상담"],
      "requiredDocs": ["증거자료", "근로계약서"],
      "legalBasis": "노무사법",
      "website": "",
...
**⚠️ 매우 중요: 위 예시의 summary 필드 형식을 정확히 따라야 합니다:**
- 섹션 헤더는 반드시 `## 상황 분석의 결과`, `## 법적 관점에서 본 현재상황`, `## 지금 당장 할 수 있는 행동`, `## 이렇게 말해보세요`로 작성하세요.
- 각 섹션 사이에는 빈 줄(`\\n\\n`)을 두세요.
- 개행 문자는 `\\n`으로 이스케이프하세요.
"""
    return prompt
