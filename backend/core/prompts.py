...
3. **scripts 필드 (필수):**
   ...
   - **⚠️ 매우 중요: body는 반드시 1인칭(저는, 제가, 저에게 등)으로 작성하세요. "사용자님은", "사용자가" 같은 3인칭 표현을 절대 사용하지 마세요.**
   ...
   - JSON 형식:
     ...
     "scripts": {{
       "to_company": {{
         "subject": "[근로계약 검토 요청] 대금 지급 및 지연이자 규정 확인 부탁드립니다",
         "body": "안녕하세요.\\n\\n현재 계약서의 대금 지급 지연 시 이자 규정이 불명확한 부분이 있어 문의드립니다. 표준 하도급계약서의 지연이자 규정을 참고하여 확인하고 싶습니다.\\n\\n관련하여 검토 후 회신 부탁드립니다."
       }},
       "to_advisor": {{
         "subject": "[노무 상담 요청] 대금 지급 및 지연이자 규정 관련 문의",
         "body": "안녕하세요. 근로계약 관련 상담을 받고자 문의드립니다.\\n\\n현재 정규직으로 주 40시간 근무하며 월급을 받고 있습니다. 계약서의 대금 지급 지연 시 이자 규정이 불명확하여 표준 하도급계약서와 비교하여 적법성 여부를 확인하고 싶습니다."
       }}
     }}
     ```
   - **to_company.subject**: 회사 인사/노무/팀장 등에게 보내는 공식·정중한 문의 메일 제목 (50자 이내)
   - **to_company.body**: **⚠️ 필수 작성** - 순수 이메일 본문 형식 (마크다운, 이모지, 아이콘 금지). **반드시 1인칭으로 작성** (저는, 제가 등). 현재 상황 요약, 문제가 되는 계약/조항 내용, criteria에서 참고한 표준계약서·법령과의 차이, 회사에 바라는 조치를 정중히 요청하는 내용. 개행은 `\\n`으로 표현. 200자 이내.
   - **to_advisor.subject**: 노무사·청년노동센터·고용노동부 등에 보내는 상담 요청 메일 제목 (50자 이내)
   - **to_advisor.body**: **⚠️ 필수 작성** - 순수 이메일 본문 형식 (마크다운, 이모지, 아이콘 금지). **반드시 1인칭으로 작성** (저는, 제가 등). 사실관계(근로형태, 근무시간, 임금 지급 방식), 문제로 의심되는 조항, criteria에서 참고한 표준계약서/법령, 상담 받고 싶은 쟁점을 자연스럽게 서술. 개행은 `\\n`으로 표현. 200자 이내.
   - **중요**: 위의 "법적 근거 (criteria 및 scripts 작성용)" 섹션에서 제공된 문서 제목(title), snippet, source_type을 활용하여 구체적인 법적 근거를 포함하세요.
   - **⚠️ 매우 중요: 본문(body)은 반드시 200자 이내로 간결하게 작성하세요. 빈 문자열("")로 두지 마세요.**
   - **⚠️ 매우 중요: body에는 마크다운(**, ##, - 등), 이모지(📊, ⚖️ 등), 아이콘을 절대 사용하지 마세요. 순수 텍스트 이메일 본문만 작성하세요.**
   - **⚠️ 매우 중요: body는 실제 사용자가 직접 작성하는 메일처럼 자연스럽게 작성하세요. "사용자님은", "사용자가" 같은 3인칭 표현을 절대 사용하지 마세요.**
...
- **⚠️ 매우 중요: scripts 필드의 body는 반드시 작성해야 합니다. 빈 문자열("")로 두지 마세요.**
  - `to_company.body`: 순수 이메일 본문 형식 (마크다운, 이모지, 아이콘 금지). **반드시 1인칭으로 작성** (저는, 제가 등). 현재 상황 요약, 문제가 되는 계약/조항 내용, criteria에서 참고한 표준계약서·법령과의 차이, 회사에 바라는 조치를 정중히 요청하는 내용을 포함하여 200자 이내로 작성하세요. 개행은 `\\n`으로 표현하세요. **"사용자님은", "사용자가" 같은 3인칭 표현을 절대 사용하지 마세요.**
  - `to_advisor.body`: 순수 이메일 본문 형식 (마크다운, 이모지, 아이콘 금지). **반드시 1인칭으로 작성** (저는, 제가 등). 사실관계(근로형태, 근무시간, 임금 지급 방식), 문제로 의심되는 조항, criteria에서 참고한 표준계약서/법령, 상담 받고 싶은 쟁점을 자연스럽게 서술. 200자 이내로 작성하세요. 개행은 `\\n`으로 표현하세요. **"사용자님은", "사용자가" 같은 3인칭 표현을 절대 사용하지 마세요.**
