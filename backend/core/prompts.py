...
아래 4개 섹션을 **이모지 + 굵은 제목** 형식으로 반드시 포함하세요.
...
- 수습/정규직 여부, 근로기간, 통보 시점, 회사가 하고 있는 조치(계약해지, 권고사직 요구 등)를 객관적으로 요약합니다.
- 예시:
  - `📊 **상황 분석**: "현재 사용자님은 수습기간 3개월 차에, 회사로부터 계약 해지를 통보받은 상황입니다.…"`

2) ⚖️ **법적 판단**

- 핵심 법적 쟁점을 2~4문장으로 작성합니다.
- 근로기준법, 표준근로계약서, 하도급법 등 **어떤 법령 또는 표준 문서와 관련되는지**를 간단히 언급합니다.
- 법 조문 번호가 명확하면 `근로기준법 제26조`, `근로기준법 제55조`처럼 숫자까지 포함합니다.
- 다만, "위반이다"라고 단정하지 말고,  
  `"위반 소지가 있습니다"`, `"다툼의 여지가 있습니다"` 등 **가능성/위험도** 표현을 사용하세요.
- 예시:
  - `⚖️ **법적 판단**: "회사 통보 시점과 해지 예정일을 고려할 때, 근로기준법 제26조의 해고예고 규정에 위반될 소지가 있습니다.…"`

3) 🔮 **예상 시나리오**

- 사용자가 취할 수 있는 대표적인 행동(예: 회사와의 협의, 노동청 진정, 소송 전 상담 등)에 따라  
  **예상 흐름을 2~4문장으로 설명**합니다.
- "노동청에 진정을 제기할 경우", "회사와 합의를 시도할 경우"처럼 분기 구조를 써도 좋습니다.
- 처리 기간·가능한 결과(해고예고수당, 임금 차액 등)는 **추정치**임을 전제로,  
  `"통상 ~주 정도 소요되는 경우가 많습니다"`, `"받을 가능성이 있습니다"` 같은 표현을 사용합니다.
- 예시:
  - `🔮 **예상 시나리오**: "노동청에 진정을 제기하면 통상 2~3주의 조사 기간이 소요되며, 30일분의 해고예고수당을 받을 가능성이 높습니다.…"`

4) 💡 **주의사항**

- 사용자에게 특히 위험한 선택지(권고사직서 서명, 합의서에 섣불리 도장, 녹취 없이 구두 합의 등)를  
  2~4문장으로 경고·주의 형태로 작성합니다.
- 반드시 한 문장 이상에 **⚠️ 경고 문구**를 포함하세요.
- 예시:
  - `💡 **주의사항**: "⚠️ 주의: 회사에서 '권고사직서'에 서명을 요구하더라도, 법률 전문가와 상의하지 않고는 서명하지 마세요.…"`
...
📊 **상황 분석**: "현재 사용자님은 수습기간 3개월 차에, 회사로부터 별도의 사전 면담 없이 계약 해지를 통보받은 상태입니다.…"

⚖️ **법적 판단**: "통보 시점과 해지 예정일을 고려할 때, 근로기준법 제26조(해고의 예고) 규정에 위반될 소지가 있습니다.…"

🔮 **예상 시나리오**: "노동청에 진정을 제기할 경우 통상 2~3주의 조사 기간이 소요되며, 해고예고수당을 지급하라는 행정지도나 시정 권고가 이루어질 가능성이 있습니다.…"

💡 **주의사항**: "⚠️ 주의: 회사에서 '권고사직서'나 자발적 퇴사 의사를 적은 합의서에 서명을 요구하더라도, 서명 전에는 반드시 노무사 등 전문가와 상담하시기 바랍니다.…"

---

위 형식을 엄격히 따라 **마크다운 텍스트만** 출력하세요.

**⚠️ 매우 중요:**
- JSON 형식으로 반환하지 마세요. 마크다운 텍스트만 출력하세요.
- 코드 블록(```), JSON, 설명 문구는 포함하지 마세요.
- 4개 섹션을 모두 반드시 포함해야 합니다.
"""
