...
- **계약서 뷰어 UI 개선**:
  - 선택된 조항 강조: 진한 배경색, 강조 테두리, 상단 파란 바
  - 조항 카드 칩화: 컴팩트한 pill 버튼 형태로 공간 효율성 향상
  - 안내 문구 시각적 강조: "빨간색/주황색으로 표시된 조항부터 먼저 확인해 주세요" 아이콘 + 볼드 텍스트
  - 카테고리 칩 통합: 같은 카테고리의 위험 개수를 합산하여 표시 (예: "보수·수당 3")
...
**2패널 레이아웃**:
- 왼쪽: AI 법률 상담 챗 (계약서 뷰어를 덮는 오버레이)
- 오른쪽: 계약서 분석 요약 패널
- 질문 → 바로 근거 확인 플로우 최적화

**구조화된 답변 탭 시스템**:
- **요약 탭**: 조항 전체 요약 및 핵심 정보
- **위험 포인트 탭** (기본 탭):
  - 위험도 배지 + 한 줄 요약
  - "왜 위험한지" 3줄 bullet 포인트
  - "다음 단계: 협상 멘트 보기" 버튼으로 자연스러운 플로우 연결
- **협상 전략 탭**:
  - 조항 수정 제안 (기존/수정안 비교, 줄바꿈 개선)
  - 사업주에게 말할 멘트 예시 (말풍선 아이콘, 번호 표시)
  - 복사 버튼으로 즉시 활용 가능
- **체크리스트 탭**:
  - 인터랙티브 체크박스 (클릭 가능)
  - 체크 시 취소선 표시
  - 실시간 피드백: 미충족 항목 50% 이상 시 위험 경고

**참고 법령/표준 계약 영역**:
- 탭 공통 하단에 위치 (모든 탭에서 동일하게 표시)
- 기본: 제목 + 한 줄 요약만 표시
- "자세히 보기" 클릭 시 전문 펼침
- "간략히 보기"로 다시 접기

**토글 가능한 사이드바**:
- 왼쪽 하단 원형 버튼으로 챗 열기/닫기
- 계약서 뷰어를 덮는 오버레이 방식
- 반응형 너비: 모바일 전체, 태블릿 90vw, 데스크톱 50%
...
#### 7. 즉시 상담 페이지 (`/legal/assist/quick`) - 개선됨
- **GPT 스타일 입력창**:
  - 말풍선 바 형태의 입력 인터페이스
  - 왼쪽: + 버튼 (리포트 불러오기 메뉴)
  - 가운데: textarea (한 줄에서 자동 확장)
  - 오른쪽: 글자수 표시 + 동그란 전송 버튼
- **컨텍스트 전환 기능**:
  - 일반 챗 모드 (컨텍스트 없음)
  - 상황 분석 리포트 컨텍스트 모드
  - 계약서 분석 리포트 컨텍스트 모드
  - 같은 세션 내에서 컨텍스트 전환 가능
- **계약서 위험도 분석 카드 렌더링**:
  - JSON 형식 응답을 자동으로 파싱하여 카드 형태로 표시
  - 위험도, 핵심 위험 포인트, 체크리스트, 수정 포인트, 법적 근거를 구조화된 카드로 표시
  - 파싱 실패 시 기존 Markdown 렌더링으로 폴백
- **새로운 채팅 시스템**:
  - `legal_chat_sessions`: 챗 세션 관리
  - `legal_chat_messages`: 메시지 저장 (컨텍스트 정보 포함)
  - 대화 내역에 첫 메시지 내용 표시
- **UI/UX 개선**:
  - 입력창을 화면의 주인공으로 배치
  - 컨텍스트 선택을 옵션으로 간소화
...
#### 9. 상황별 법률 분석 페이지 (`/legal/situation`)
- **상황 템플릿 제공**: 
...
#### 즉시 상담 페이지 개선 (2025-11-30)
- ✅ **GPT 스타일 입력창**: 말풍선 바 형태의 입력 인터페이스 구현
- ✅ **컨텍스트 전환 기능**: 일반 챗, 상황 분석 리포트, 계약서 분석 리포트 컨텍스트 전환 지원
- ✅ **새로운 채팅 시스템**: `legal_chat_sessions`와 `legal_chat_messages` 테이블로 통합 관리
- ✅ **계약서 위험도 분석 카드 렌더링**: JSON 응답을 자동 파싱하여 구조화된 카드로 표시
- ✅ **UI/UX 개선**: 입력창 우선순위 강화, 컨텍스트 선택 간소화, 헤더 정리
- ✅ **대화 내역 개선**: 첫 메시지 내용을 대화 내역에 표시
- ✅ **백엔드 API 개선**: 세션 생성 API를 body로 받도록 수정, 상황 분석 워크플로우 오류 수정

#### 계약서 전문 뷰어 레이아웃 개선 (2025-01-XX)
- ✅ **양옆 잘림 문제 해결**: 계약서 전문 섹션의 가로 스크롤 및 잘림 현상 수정
  - `overflowX: 'hidden'` 제거로 내용이 잘리지 않도록 개선
  - 중복 padding 제거로 레이아웃 정상화
  - 계약서 전문 뷰어의 반응형 레이아웃 최적화

#### AI 법률 상담 UI 사용성 개선 (2025-11-18)
